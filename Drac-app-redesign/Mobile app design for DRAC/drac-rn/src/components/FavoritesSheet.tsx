...
export function FavoritesSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
            ...
            {mockCameras.map((cam) => {
              ...
              return (
                <View key={cam.id} style={[styles.row, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                  <LinearGradient colors={cam.tint} style={styles.thumb} />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.name, { color: theme.text }]}>{cam.name}</Text>
                    <Text style={[styles.area, { color: theme.textSub }]}>{cam.area}</Text>
                  </View>
                  <Pressable
