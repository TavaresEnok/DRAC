...
import { MOCK_EVENTS, MOCK_CAMERAS, VMSEvent } from '../data/mockData';
...
const EVENT_TYPE_LABELS: Record<string, string> = {
  ...
  ptz_tour_started: 'PTZ Tour Started',
...
export default function EventsPage() {
  ...
  const [drawerEvent, setDrawerEvent] = useState<VMSEvent | null>(MOCK_EVENTS[0] ?? null);

  const filtered = MOCK_EVENTS.filter(evt => {
  ...
  const current = drawerEvent ?? pageData[0] ?? filtered[0] ?? MOCK_EVENTS[0];
  const currentCamera = current ? MOCK_CAMERAS.find(c => c.id === current.cameraId) : undefined;
        ...
        <div className="flex min-h-0 flex-col rounded-2xl border border-card-border bg-card overflow-hidden">
          <div className="px-4 pt-4 pb-3 border-b border-border space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Select value={sevFilter} onValueChange={v => { setSevFilter(v); setPage(0); }}>
                ...
                <SelectContent>
                  {['All', 'critical', 'warning', 'info'].map(s => <SelectItem key={s} value={s} className="text-xs capitalize">{s === 'All' ? 'Todas' : s}</SelectItem>)}
                </SelectContent>
              </Select>
              ...
              <Select value={ackFilter} onValueChange={v => { setAckFilter(v); setPage(0); }}>
                <SelectTrigger className="w-32 h-8 text-[11px]"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {['All', 'Unacknowledged', 'Acknowledged'].map(s => <SelectItem key={s} value={s} className="text-xs">{s === 'All' ? 'Status' : s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex items-center justify-between border-t border-border px-4 py-3 bg-card">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="px-2.5 py-1.5 rounded-lg border border-border text-[11px] disabled:opacity-40 hover:bg-[hsl(var(--accent))] transition-colors">Prev</button>
              <button onClick={() => setPage(p => Math.min(pages - 1, p + 1))} disabled={page >= pages - 1} className="px-2.5 py-1.5 rounded-lg border border-border text-[11px] disabled:opacity-40 hover:bg-[hsl(var(--accent))] transition-colors">Next</button>
            </div>
          </div>
        </div>
              ...
              <div className="p-4 space-y-4 overflow-y-auto">
                ...
                <div className="rounded-2xl border border-border overflow-hidden bg-[hsl(220_20%_7%)]">
                  <div className="aspect-video relative">
                    ...
                    <div className="absolute left-3 top-3 rounded bg-black/40 px-1.5 py-0.5 text-[9px] font-mono text-white/70">LIVE</div>
