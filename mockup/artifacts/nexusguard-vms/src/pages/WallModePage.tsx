...
export default function WallModePage() {
  ...
  const displayCams = MOCK_CAMERAS.slice(0, layout.count);
  const activeAlarms = MOCK_CAMERAS.filter(c => c.status === 'alarm').length;
          ...
          <div className="flex items-center gap-4">
            {activeAlarms > 0 && (
              <div className="flex items-center gap-1.5 alarm-blink">
                ...
                <span className="text-xs text-red-400 font-mono">{activeAlarms} Active Alarms</span>
              </div>
            )}
