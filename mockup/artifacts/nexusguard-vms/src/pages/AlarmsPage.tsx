...
const ALARM_STATS = [
  { zone: 'Perimeter', count: 12 },
  { zone: 'Lobby',     count: 8 },
  { zone: 'Parking',   count: 6 },
  { zone: 'Access',    count: 9 },
  { zone: 'Server',    count: 3 },
  { zone: 'Executive', count: 2 },
];
            ...
            <div className="px-4 pb-3 border-t border-border pt-3 space-y-3">
              ...
              {showNoteInput && (
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={() => { if (noteInput.trim()) { onAddNote?.(noteInput.trim()); setNoteInput(''); setShowNoteInput(false); } }} className="h-8 px-3 rounded bg-primary text-primary-foreground text-xs">Add</button>
                </div>
              )}
...
export default function AlarmsPage() {
  ...
  const ackAlarms      = alarms.filter(a => a.status === 'acknowledged');
    ...
    <div className="flex h-full min-h-0 gap-0">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 min-w-0">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className={`w-2 h-2 rounded-full ${activeAlarms.length > 0 ? 'status-alarm rec-pulse' : 'status-online'}`} />
          ...
          >
            ...
            {muted ? 'Unmute Alarms' : 'Mute Alarms'}
          </button>
        </div>
        ...
        {activeAlarms.length > 0 ? (
            ...
            <AnimatePresence>
              {activeAlarms.map(alarm => (
                <AlarmCard
                  key={alarm.id}
                  alarm={alarm}
                  onAck={() => acknowledgeAlarm(alarm.id, user?.name ?? 'Unknown')}
          ...
          <div className="bg-card border border-card-border rounded-lg p-8 text-center">
            ...
            <div className="text-[13px] font-semibold text-[hsl(var(--chart-3))]">All Clear</div>
            <div className="text-[11px] text-[hsl(var(--muted-foreground))] mt-1">No active alarms</div>
          </div>
        )}
        ...
        {ackAlarms.length > 0 && (
          <div className="space-y-2">
            <div className="text-[10px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider flex items-center gap-2">
              <CheckCheck className="w-3 h-3" />
              Acknowledged ({ackAlarms.length})
            </div>
            {ackAlarms.slice(0, 5).map(alarm => (
              <AlarmCard
                key={alarm.id}
                alarm={alarm}
                onAck={() => acknowledgeAlarm(alarm.id, user?.name ?? 'Unknown')}
              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-border">
                    {['Priority','Name','Zone','Resolved by','Triggered'].map(h => (
      ...
      <div className="w-60 border-l border-border bg-card p-4 overflow-y-auto shrink-0 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'Active',   value: activeAlarms.length,  color: 'text-[hsl(354,52%,62%)]' },
            { label: "Ack'd",    value: ackAlarms.length,     color: 'text-[hsl(38,58%,60%)]' },
            { label: 'Resolved', value: resolvedAlarms.length, color: 'text-[hsl(152,36%,52%)]' },
        ...
        <div>
          <h4 className="text-[9px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))] mb-2">By Zone</h4>
          <ResponsiveContainer width="100%" height={130}>
            <BarChart data={ALARM_STATS} layout="vertical" margin={{ top: 0, right: 4, left: 0, bottom: 0 }}>
        ...
        <div>
          <h4 className="text-[9px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))] mb-2">By Priority</h4>
          <div className="space-y-2">
